-- 配置表：存放需对比的对象名册
CREATE TABLE IF NOT EXISTS `cfg_compare_objects` (
  ...
  `is_active` tinyint NOT NULL DEFAULT '1' COMMENT '是否开启对比: 1是，0否',
  `need_sync` tinyint NOT NULL DEFAULT '0' COMMENT '是否需要同步数据: 1是，0否',
  `date_column` varchar(100) DEFAULT NULL COMMENT '增量同步的日期字段名，为NULL表示全量同步',
...
CREATE TABLE IF NOT EXISTS `cfg_compare_diff` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
